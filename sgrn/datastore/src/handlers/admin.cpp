...
Task<HttpResponsePtr> AdminApiHandler::handleListIotObjects(HttpRequestPtr tsp_req) {
    const Json::Value& session = tsp_req->attributes()->get<Json::Value>("session_json");
    const std::string org = session["user"]["organisation"].asString();
    ...
    try {
        auto res = co_await db->execSqlCoro(
            "SELECT o.id, o.automated_service_id, o.name, o.metadata, o.status, o.domain, o.created_at, a.name as automated_service_name "
            "FROM telemetry.\"object\" o "
            "JOIN core.automated_services a ON o.automated_service_id = a.id "
            "WHERE o.organisation = $1 AND (o.domain = $2 OR $2 IS NULL OR $2 = '') ORDER BY o.id",
            org, domain);
        ...
        for (const auto& row : res) {
            ...
            obj["automated_service_id"] = row["automated_service_id"].as<int32_t>();
            obj["automated_service_name"] = row["automated_service_name"].as<std::string>();
            obj["name"] = row["name"].as<std::string>();
            obj["status"] = row["status"].as<std::string>();
            obj["domain"] = row["domain"].isNull() ? "" : row["domain"].as<std::string>();
            obj["created_at"] = row["created_at"].as<std::string>();
            obj["metadata"] = parseMetadataValue(row["metadata"].as<std::string>());
            objects.append(obj);
        }
...
Task<HttpResponsePtr> AdminApiHandler::handleUpdateIotObjectMetadata(HttpRequestPtr tsp_req) {
    const std::string id_str = tsp_req->getParameter("id");
    auto json = tsp_req->getJsonObject();
    if (id_str.empty() || !json || !json->isMember("metadata")) {
        co_return createErrorResponse("id and metadata are required", k400BadRequest);
    ...
    try {
        ...
        // Update with ORG check via JOIN to automated services.
        auto res = co_await db->execSqlCoro("UPDATE telemetry.\"object\" "
                                            "SET metadata = metadata || $1::jsonb "
                                            "WHERE id = $2 AND organisation = $3",
            meta_str, obj_id, org);
