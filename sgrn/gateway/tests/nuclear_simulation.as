// ============================================================================
// nuclear_simulation.as — Pressurized Water Reactor (PWR) Simulation Script
//
// Simulates the physical behavior of a 1000 MWe PWR nuclear power plant,
// including neutronics, thermal-hydraulics, steam generator level control,
// turbine frequency/load synchronization, and containment systems.
//
// Prerequisites:
//   env.as must be loaded first (either via the shell init-script or by calling
//   runScript("env.as") manually). env.as provides setupEnv(), plc, db10-db16.
//
// Standalone usage:
//   s7> runScript("env.as")
//   s7> runScript("nuclear_simulation.as")
//
//   OR just:  s7> runScript("nuclear_simulation.as")
//   (main() calls setupEnv() with defaults if handles are null)
// ============================================================================
...
double simulation_hz = 10.0; // Customizable update rate (1.0 = 1 Hz)
...
void main() {
    ...
    while (true) {
        ...
        writeReactorCore(db10, ts);
        writePrimaryCoolant(db11, ts);
        writeSteamGenerator(db12, ts);
        writeTurbine(db13, ts);
        writeSafetySystems(db14, ts);
        writeRadMonitoring(db15, ts);
        writeWasteProcessing(db16, ts);

        // 3. Periodic report
        if (iteration % int(Math::max(1.0, 5.0 * simulation_hz)) == 0) {
            printReport(ts, iteration);
        }

        iteration++;
        sleep(int(1000.0 / simulation_hz));
    }
}
...
void simulatePhysics() {
    ...
    if (scrammed) {
        ...
        reactor_power_mw = reactor_power_mw * 0.5;
        if (reactor_power_mw < 1.0) reactor_power_mw = 0.0;
    } else {
        // Normal operation: reactivity balanced by rod position and boron
        double rod_reactivity = (rod_bank_pos - 75.0) * 8.0;
        double boron_reactivity = (1200.0 - boron_ppm) * 1.5;
        reactivity = rod_reactivity + boron_reactivity;
    ...
    t_outlet = t_inlet + (heating_rate * 33.0);
...
void writePrimaryCoolant(PrimaryCoolant@ db, DTL@ ts) {
    ...
    for (int i = 0; i < 4; i++) {
        db.rcp[i].running = true;
        db.rcp[i].speed_rpm = 1485;
        db.rcp[i].flow_m3h = 24000.0f;
        db.rcp[i].delta_p_bar = 6.2f;
